...
.btn-wa { background: #25D366; color: #fff; border: none; border-radius: 8px; padding: 16px 40px; font-size: 18px; font-weight: 600; cursor: pointer; transition: all 0.2s; display: inline-block; }
.btn-wa:hover { background: #20bd5a; transform: translateY(-1px); }
.btn-wa:disabled { background: #333; color: #666; cursor: not-allowed; transform: none; }
...
<p class="subtitle">Connect your WhatsApp Business App number to the Cloud API.<br>Keep using the app normally - messages sync to both.</p>
{% if not config_id %}
<div class="no-config">
<strong>Configuration ID needed.</strong><br>
Set the WA_CONFIG_ID environment variable on Render.
</div>
{% endif %}
<div class="steps">
...
<li>Get a verification code - enter it in your WhatsApp Business App</li>
<li>Confirm in the app to share chat history</li>
<li>Done! Your number works on both the app AND the API</li>
</ol>
</div>
<button id="signup-btn" class="btn-wa" onclick="doSignup()" {% if not config_id %}disabled{% endif %}>Connect WhatsApp Business</button>
    ...
    try {
        ...
        if (data.type === 'WA_EMBEDDED_SIGNUP') {
            if (data.event === 'FINISH' || data.event === 'FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING') {
                var phoneId = data.data ? data.data.phone_number_id : '';
                var wabaId = data.data ? data.data.waba_id : '';
                setStatus('success', 'WhatsApp connected! Phone ID: ' + phoneId + ' WABA ID: ' + wabaId);
                fetch('/api/wa-signup-complete', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({phone_number_id: phoneId, waba_id: wabaId, event: data.event})
                });
            } else if (data.event === 'CANCEL') {
                setStatus('error', 'Setup was cancelled.');
            }
        }
    } catch (e) {}
});
...
function onFBLogin(response) {
    if (response.authResponse) {
        ...
        setStatus('info', 'Exchanging token... please wait.');
        ...
        .then(function(data) {
            if (data.success) {
                setStatus('success', 'Token exchanged successfully! WhatsApp Cloud API is now active.');
            } else {
                setStatus('error', 'Token exchange failed: ' + (data.error || 'Unknown error'));
            }
        })
        .catch(function(err) {
        ...
        setStatus('error', 'Login was not completed.');
    }
}

function doSignup() {
    if (!CONFIG_ID) {
        setStatus('error', 'Configuration ID is not set.');
        return;
    }
    FB.login(onFBLogin, {
        config_id: CONFIG_ID,
...
def health():
    ...
    return jsonify({
        ...
        "wa_configured": bool(WA_CONFIG_ID),
    })
...
def exchange_token():
    ...
    try:
        ...
        if "access_token" in result:
            ...
            log.error("Token exchange failed: %s", result)
...
def wa_signup_complete():
    data = request.get_json()
    log.info("WhatsApp signup complete: %s", json.dumps(data, indent=2))
    state_file = os.path.join(DATA_DIR, "wa_signup.json")
    with open(state_file, "w") as f:
        json.dump(dict(data, completed_at=datetime.now(timezone.utc).isoformat()), f, indent=2)
...
def webhook_verify():
    ...
    if mode == "subscribe" and token == WEBHOOK_VERIFY_TOKEN:
        log.info("Webhook verified! Challenge: %s", challenge)
        return challenge, 200
    else:
        log.warning("Webhook verification failed. Token: %s", token)
...
def webhook_receive():
    ...
    log.info("Webhook received: object=%s, entries=%d", obj_type, len(entries))
    for entry in entries:
        if obj_type == "whatsapp_business_account":
            ...
            log.info("Unknown webhook object type: %s", obj_type)
    return "OK", 200
        ...
        if field == "messages":
            ...
            for msg in messages:
                ...
                if msg.get("type") == "text":
                    ...
                    text = "[" + msg.get("type", "unknown") + "]"
                msg_record = {
                    "platform": "whatsapp",
                    "received_at": datetime.now(timezone.utc).isoformat(),
                    "timestamp": msg.get("timestamp", ""),
                    "from_number": wa_id,
                    "from_name": contact_map.get(wa_id, ""),
                ...
                log.info("WA message from %s (%s): %s [total: %d]", msg_record["from_name"], wa_id, text[:80], count)
        elif field == "history":
            ...
            log.info("WA webhook field: %s", field)
...
def process_instagram_entry(entry):
    ...
    for event in messaging:
        ...
        if message:
            ...
            log.info("IG message (%s) from %s: %s [total: %d]", direction, sender.get("id", "?"), msg_record["text"][:80], count)
    for change in entry.get("changes", []):
        log.info("IG change field: %s", change.get("field", ""))
...
def process_messenger_entry(entry):
    ...
    for event in messaging:
        ...
        if message:
            ...
            count = save_message(msg_record)
            direction = "echo/sent" if msg_record["is_echo"] else "received"
            log.info("FB message (%s) from %s: %s [total: %d]", direction, sender.get("id", "?"), msg_record["text"][:80], count)
...
if __name__ == "__main__":
    ...
    log.info("Starting OpenClaw Webhook Receiver on port %d", port)
